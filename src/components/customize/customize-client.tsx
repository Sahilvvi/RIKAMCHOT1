...
export function CustomizeClient({ product }: { product: StorefrontProduct }) {
  ...
  const base = product.variants[0]?.price || product.price || 6999;
  const total = text ? base + 499 : base;
  const colorName = colors.find((c) => c.hex === color)?.name || "Custom";

  function handleAddToBag() {
    const variant = product.variants[0];
    addItem({
      productId: product.id,
      variantId: variant?.id || `${product.id}-custom`,
      name: text ? `${product.name} · ${text}` : product.name,
      slug: product.slug,
      image: product.image,
      size: text || "Custom",
      color: colorName,
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-2">
          ...
          >
            ...
            <div className="rounded-2xl border border-border bg-card p-6">
              ...
              <Button size="lg" className="mt-6 w-full" onClick={handleAddToBag}>
